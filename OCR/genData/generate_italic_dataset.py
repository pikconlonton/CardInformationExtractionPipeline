...
def load_fonts(fonts_dir: Path) -> list:
    ...
    if not font_groups:
        ...
        print(f"[INFO] Loaded {len(font_groups)} font families")
...
def render_sample(text: str, fonts: list, target_height: int = 64) -> Image.Image:
    ...
    # Ước lượng shear max để tránh tràn lề
    max_shear = 0.35

    extra_w = int(abs(max_shear) * text_h)

    width = max(32, text_w + pad_x * 2 + extra_w)
    ...
    # ── Create text layer (for transform) ─────────────────────
    ...
    if mode == "right":
        shear = random.uniform(0.15, 0.35)
    elif mode == "left":
        shear = random.uniform(-0.35, -0.15)
    else:
        shear = 0.0

# 👉 APPLY TRANSFORM Ở NGOÀI
    if shear != 0.0:
        shift_x = int(abs(shear) * text_h / 2)

        text_layer = text_layer.transform(
            (width, height),
            Image.AFFINE,
            (1, shear, -shift_x if shear > 0 else shift_x, 0, 1, 0),
